...
> "The vulnerability is in `auth_handler.py`, in the `validate()` function. Inadequate rate limiting and token validation expose it to brute force attacks and unauthorized access."
...
$ # There is no validate() with rate limiting.
$ # There is no token validation logic described.
```
...
sequenceDiagram
    ...
    LLM->>You: "auth_handler.py, validate(),<br/>no rate limiting, token not validated"
    Note over You: Sounds right...
    You->>You: checks the actual codebase
    Note over You: auth_handler.py doesn't exist.
    Note over You: validate() doesn't exist.
...
Your Chapter 0 setup stays exactly as-is — no new dependencies, no new config.

```
# Wraps the LLM client — one class, one method, one job: send a prompt, get a response
class OneShot:
    llm_client: LLMClient       # injected at construction — swap providers or mock in tests

    function send(prompt):      # one prompt in, one Response out
        response = llm_client.complete(
            model: config.model,
            messages: [{ role: "user", content: prompt }],  # single user turn — no history
            max_tokens: config.max_tokens
        )
        return Response(
            content: response.text,                          # the LLM's raw answer
            model: config.model,                             # which model answered
...
> Implement `OneShot` in `tbh_code/oneshot.py` following the pseudocode above.
> Wire `--mode oneshot` in `tbh_code/main.py` to create an `OneShot` instance,
> call `send()` with the task, and print the response content.
> Use the existing `tbh_code/llm.py` client — don't add new dependencies.

### Step 3 — Re-run and see what comes back

Run the same command again:

```
$ uv run tbh-code --mode oneshot --task "Find the security vulnerability in the todo-api authentication system. Identify the file, the function, and explain what's wrong."
```

This time it calls the LLM. You'll see something like this — here's a real response from `gpt-4o-mini`:

```
The security vulnerability in the TODO API's authentication system is in the
auth_handler.py file, specifically in the validate() function, where inadequate rate
limiting and token validation expose it to brute force attacks and
unauthorized access.
```

> **A note on models.** Newer reasoning models (o3, Claude Sonnet, Gemini 2.5) sometimes produce a more carefully hedged or even partially correct answer here — especially if your setup routes calls through a companion agent that can read local files. That's not the point of this chapter.
>
> This book is not about making the LLM model smarter. It's about building agent intelligence — the loop, the tools, the reflection layer that makes *any* model more reliable regardless of its raw capability. A better model narrows the gap. A better architecture closes it. We're building the architecture.
>
> Use whatever model you have. If your answer looks more specific than the one above, great — note it down. The lesson still holds: **the one-shot answer, however confident, is unverified.**

### Step 4 — Check every claim

Now look at what came back. Not the quality of the prose. The specifics.

Open `todo-api/` and check each claim:

- Does `auth_handler.py` exist?
- Does `validate()` have a rate limiting implementation?
- Is there any token validation logic at all?
...
No `auth_handler.py`. No `validate()`. The LLM named files and functions that don't exist, described problems that may or may not be real, and delivered it all with the confidence of someone who had actually read the code.

Fluent. Specific. Unverified.

Save your output. You'll compare it in a few minutes when you run the loop version against the same task.
...
**Specificity.** The answer names `auth_handler.py` and `validate()`. The reflect phase can't check if they exist (no tools yet), but it can notice that the answer is *suspiciously specific* for a system with no file access. Naming concrete things you've never seen is a hallucination signal.
...
```
# runs after every act() — loop again or accept?
reflect(task, result) → ReflectOutcome

ReflectOutcome:
    should_continue: bool        # true = iterate; false = done
    confidence: float (0.0-1.0)  # 0 = guessing; 1 = grounded
    issues: string[]             # fed back into next observe()
```
...
```
# Wraps OneShot with a reflect-and-retry loop
AgentLoop:
    llm_client: LLMClient       # same client as OneShot — no new dependencies
    max_iterations: int = 5     # safety cap — prevents infinite loops
    run(task) → AgentResult     # replaces OneShot.send() as the top-level call

# Everything the caller gets back — answer plus the full reasoning trail
AgentResult:
    answer: string              # final answer after reflect() accepted it
    iterations: int             # how many loop cycles it took
    trace: StepTrace[]          # one entry per phase per iteration — the audit log

# One phase of one iteration — the building block of the trace
StepTrace:
    step: int                   # iteration number (1-based)
    phase: enum("observe", "think", "act", "reflect")
    content: string             # what the LLM produced in this phase
    grounded: bool              # did reflect() consider this step verified?
```
...
[loop] Iteration 1:
  ...
  [act]     The authentication system likely has a vulnerability in token
            ...
            auth_handler.py or middleware/auth_handler.py and the function might be
            validate() or check_auth().
...
| Function names | Invented `validate()`          | Did not invent any                           |
...
The full spec for this chapter lives in [spec/ch01/](../spec/ch01/):

📁 [spec/ch01/](../spec/ch01/)

| File | Description |
|------|-------------|
| [prompt-template.md](../spec/ch01/prompt-template.md) | What to build (language-agnostic) |
| [interface-spec.md](../spec/ch01/interface-spec.md) | OneShot and AgentLoop contracts |
| [expected-output.txt](../spec/ch01/expected-output.txt) | Full expected output for both modes |
| [test_ch01.py](../spec/ch01/validation/test_ch01.py) | Automated tests your code must pass |
...
1. **See the full picture.** Ask your agent:

   > Draw a component diagram in ASCII art showing how `main.py`, `OneShot`,
   > `AgentLoop`, `llm.py`, and `ReflectOutcome` connect for the Chapter 1 implementation.
   > Expand `AgentLoop` to show the internal observe → think → act → reflect loop
   > and how `ReflectOutcome` feeds back into the next iteration.

2. **Explore all interfaces.** Ask your agent:

   > Draw all Chapter 1 interfaces (`OneShot`, `AgentLoop`, `AgentResult`, `StepTrace`, `ReflectOutcome`)
   > as an ASCII diagram showing their fields, types, and relationships to each other.

3. **Run your one-shot on a different task.** Try: *"Refactor the task creation endpoint to use input validation."* Does it reference files that exist?
...
LLM plus memory = a chatbot which talks, it responds. A chatbot can't see your files, can't run tests, can't verify claims. 

What we build first as one-shot wrapper? Not even this. No memory. It's a chatbot minus the chat.
...
What we build first one-shot wrapper is below Level 1. The agent loop what we build is the beginning of Level 3 — but without tools, it has Level 3 structure with Level 1 capabilities. That's why it hit a ceiling. That ceiling is what Chapter 2 breaks through.
