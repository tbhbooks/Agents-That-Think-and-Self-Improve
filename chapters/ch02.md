...
```
# Holds the codebase on disk — the agent's only window into the code
FileContext:
    codebase_path: string             # root directory to scan
    files: FileEntry[]                # all loaded files
    load(path) → void                 # scan and populate files[]
    get_relevant(query) → FileEntry[] # filter to files matching the task

# One file's worth of context passed to the LLM
FileEntry:
    path: string     # relative to codebase root — what the agent cites
    content: string  # raw file text
    size: int        # bytes — used to stay within token limits
    language: string # inferred from extension (e.g. "python", "pseudo")
```
...
uv run tbh-code --codebase ./todo-api

# Single question — one answer, exit
uv run tbh-code --codebase ./todo-api --ask "Find the security vulnerability"
```
...
Tell your agent to build it:

> Implement Chapter 2 in `tbh_code/`:
> - `file_context.py` — `FileContext` and `FileEntry`: scan a codebase directory, load files, filter by relevance to a query
> - `context_selector.py` — `ContextSelector`: score and rank files by keyword match and path relevance, stay within token budget
> - `history.py` — `ConversationHistory` and `Message`: multi-turn message list with token-aware truncation
> - `structured_response.py` — `StructuredResponse`: parse LLM output into answer, confidence, sources, and follow-up questions
> - `agent.py` — `Agent` with `chat()`: wire all four components together with the Ch 1 loop in the observe phase
> - Wire `--codebase <path>` and `--task <question>` flags in `main.py`
>
> Reuse `tbh_code/llm.py` — no new LLM dependencies.

Then run the task that defeated Chapter 1:

```bash
$ uv run tbh-code --codebase ./todo-api --task "Find the security vulnerability in the todo-api authentication system. Identify the file, the function, and explain what's wrong."
```
...
Full spec for this chapter in [spec/ch02/](../spec/ch02/):

📁 [spec/ch02/](../spec/ch02/)

| File | Description |
|------|-------------|
| [prompt-template.md](../spec/ch02/prompt-template.md) | What to build (language-agnostic) |
| [interface-spec.md](../spec/ch02/interface-spec.md) | Agent, History, FileContext, StructuredResponse contracts |
| [expected-output.txt](../spec/ch02/expected-output.txt) | Single-question, multi-turn, and honesty examples |
| [test_ch02.py](../spec/ch02/validation/test_ch02.py) | 12 tests: structured output, file reading, history, honesty |
...
Run from your `tbh-code/` sibling folder:
...
1. **See the full picture.** Ask your agent:
   ...
   > Draw a component diagram in ASCII art showing how `main.py`, `Agent`, `FileContext`,
   > `ContextSelector`, `ConversationHistory`, and `StructuredResponse` connect.
   > Expand the `Agent.chat()` flow to show how observe → think → act → reflect maps
   > to file loading, prompt building, LLM call, and structured parsing in Chapter 2.

2. **Ask about something subtle.** *"Is there an N+1 query in the task routes?"* (There is — `list_tasks` loads tags in a loop.) Does the agent find it? Does it cite the right lines?

3. **Break the system prompt.** Remove the constraints. Ask the same auth vulnerability question. Does the agent hallucinate file names like the Ch 1 one-shot did?

4. **Compare structured vs raw.** Remove the JSON output format from the system prompt. Is the raw prose answer better or worse? Can your code still do anything useful with it?
