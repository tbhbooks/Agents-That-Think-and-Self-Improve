...
Full spec in `spec/ch03/`:

```
spec/ch03/
├── prompt-template.md     What to build (language-agnostic)
├── interface-spec.md      Tool, SimpleTool, MCPTool, ToolRegistry, MCP contracts
├── expected-output.txt    Tool calls, MCP discovery, verification traces
└── validation/
    └── test_ch03.py       Tests: tool interface, registry, MCP, verification
```
