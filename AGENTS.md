...
- Create the `tbh-code` directory directly in the root of the repository alongside `AGENTS.md` so the user can easily view files and execute commands as the project unfolds.
- If the reader asks to run code but the root `tbh-code` directory does not exist, guide them through Chapter 0 setup to create it here first.
