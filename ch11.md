...
Full spec for this chapter in `spec/ch11/`:

```
spec/ch11/
├── prompt-template.md     What to build (language-agnostic)
├── interface-spec.md      BroadcastBus, AgentCard, PeerRegistry,
│                          share_skill, share_improvement contracts
├── expected-output.txt    Broadcast, discovery, skill sharing,
│                          improvement propagation traces
└── validation/
    └── test_ch11.py       Tests: agents discover each other,
                           agent cards have all fields,
                           skills shared only when tools match,
                           improvements propagate only if verified
```
